...
# function findroot!(t, x, dx, g_tupl_old, g_tupl, wrapped, eventorder, tvS, xvS, gvS,
#         t0, δt_old, x_dx, x_dx_val, g_dg, g_dg_val, nrabstol,
#         newtoniter, nevents)

#     if surfacecrossing(g_tupl_old, g_tupl, eventorder) && !(wrapped)
#         #auxiliary variables
#         g_val = g_tupl[2]
...
function taylorinteg_wrap(f!, g, h!, q0::Array{U,1}, t0::T, tmax::T,
        order::Int, abstol::T, params = nothing; maxsteps::Int=500, parse_eqs::Bool=true,
        dense::Bool=true, eventorder::Int=0, newtoniter::Int=10, nrabstol::T=eps(T), show_warn::Bool=true) where {T <: Real,U <: Number}
    ...
    t = t0 + Taylor1( T, order )
    x .= Taylor1.( q0, order )
    return _taylorinteg_wrap!(Val(dense), f!, g, h!, t, x, dx, q0, t0, tmax, abstol, rv, params;
        parse_eqs, maxsteps, eventorder, newtoniter, nrabstol, show_warn)
end

function _taylorinteg_wrap!(dense::Val{D}, f!, g, h!, t::Taylor1{T}, x::Array{Taylor1{U},1}, dx::Array{Taylor1{U},1},
        q0::Array{U,1}, t0::T, tmax::T, abstol::T, rv::RetAlloc{Taylor1{U}}, params;
        parse_eqs::Bool=true, maxsteps::Int=500, eventorder::Int=0,
        newtoniter::Int=10, nrabstol::T=eps(T), show_warn::Bool=true) where {T <: Real,U <: Number, D}
...
function taylorinteg_wrap(f!, g, h!, q0::Array{U,1}, trange::AbstractVector{T},
        order::Int, abstol::T, params = nothing; maxsteps::Int=500, parse_eqs::Bool=true,
        eventorder::Int=0, newtoniter::Int=10, nrabstol::T=eps(T), show_warn::Bool=true) where {T <: Real,U <: Number}
    ...
    t = t0 + Taylor1( T, order )
    x .= Taylor1.(q0, order)
    return _taylorinteg_wrap!(f!, g, h!, t, x, dx, q0, trange, abstol, rv, params;
        parse_eqs, maxsteps, eventorder, newtoniter, nrabstol, show_warn)
end

function _taylorinteg_wrap!(f!, g, h!, t::Taylor1{T}, x::Array{Taylor1{U},1}, dx::Array{Taylor1{U},1},
        q0::Array{U,1}, trange::AbstractVector{T}, abstol::T, rv::RetAlloc{Taylor1{U}}, params;
        parse_eqs::Bool=true, maxsteps::Int=500, eventorder::Int=0, newtoniter::Int=10, nrabstol::T=eps(T), show_warn::Bool=true) where {T <: Real,U <: Number}
