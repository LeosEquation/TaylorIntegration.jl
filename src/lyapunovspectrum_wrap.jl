...
"""
    lyap_taylorinteg_wrap(f!, q0, t0, tmax, order, abstol[, jacobianfunc!=nothing];
        maxsteps::Int=500, parse_eqs::Bool=true)
    lyap_taylorinteg_wrap(f!, q0, trange, order, abstol[, jacobianfunc!=nothing];
        maxsteps::Int=500, parse_eqs::Bool=true)
...
"""
function lyap_taylorinteg_wrap(f!, h!, q0::Array{U,1}, t0::T, tmax::T,
    ...
    t = t0 + Taylor1( T, order )
    x .= Taylor1.( x0, order )
    return _lyap_taylorinteg_wrap!(f!, h!, t, x, dx, q0, t0, tmax, abstol, jt, _dv, rv,
        params, jacobianfunc!; maxsteps, parse_eqs, show_warn)
end

function _lyap_taylorinteg_wrap!(f!, h!, t::Taylor1{T}, x::Array{Taylor1{U},1}, dx::Array{Taylor1{U},1},
        ...
        maxsteps::Int=500, show_warn::Bool = true) where {T<:Real, U<:Number}
...
function lyap_taylorinteg_wrap(f!, h!, q0::Array{U,1}, trange::AbstractVector{T},
        order::Int, abstol::T, params = nothing, jacobianfunc! = nothing;
        maxsteps::Int=500, parse_eqs::Bool=true, show_warn::Bool = true) where {T<:Real, U<:Number}
    ...
    return _lyap_taylorinteg_wrap!(f!, h!, t, x, dx, q0, trange, abstol, jt, _dv, rv,
        params, jacobianfunc!; parse_eqs, maxsteps, show_warn)
